...
export default class AddToCalender extends Component<Args> {
  ...
  get googleUrl(): string {
    ...
    return `https://calendar.google.com/calendar/render?action=TEMPLATE&dates=${startTime}/${endTime}&text=${event.name}&location=${this.calenderLocation}&ctz=${event.timezone}&details=${this.description}`;
  }
  ...
  get yahooUrl(): string {
    ...
    return `https://calendar.yahoo.com/?v=60&title=${event.name}&st=${startTime}&et=${endTime}&desc=${this.description}&in_loc=${this.calenderLocation}`;
  }
  ...
  get outlookUrl(): string {
    ...
    return `https://outlook.live.com/calendar/0/deeplink/compose?subject=${event.name}&startdt=${startTime}&enddt=${endTime}&body=${(this.description).substring(0, 1000)}&location=${this.calenderLocation}`;
  }
